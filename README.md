...
Capped extreme values at the 1st and 99th percentile for each feature. A small number of very large corporate buyers were pulling the cluster centroids significantly, and clipping brought the distribution back to something representative of the typical customer base.
...
Used `StandardScaler` after the log transform (not before). The scaler was fitted on log-transformed values, so the saved scaler expects the same transformation at prediction time, which is something the Streamlit app handles correctly.
...
**Promising (21.0%):** Recent buyers who haven't yet formed a habit. The right nudge, such as a follow-up email or a loyalty incentive, can move them toward Champions.

**At Risk (24.7%):** Were good customers but haven't returned, so a win-back campaign with a time-limited discount is the standard play here.

**Lost / Inactive (33.5%):** Last purchased a long time ago with low spend, so re-engagement is expensive on this group. Better to survey, identify the recoverable ones, and suppress the rest from regular campaigns.
...
The model files are already saved in `models/`, so there is no need to re-run the notebook before using the app.
...
Data Scientist & ML Engineer. Production Engineering graduate transitioning into data science, with hands-on experience building end-to-end ML pipelines across fraud detection, healthcare risk prediction, and customer churn modelling.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Victor%20Pius-blue?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/victor-pius-4061a9332)
[![GitHub](https://img.shields.io/badge/GitHub-profpius-black?style=flat-square&logo=github)](https://github.com/profpius)
